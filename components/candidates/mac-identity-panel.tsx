...
function IdentitySelect({ name, identities, value, onChange, candidate }: { name: string; identities: MacIdentityOption[]; value: string; onChange: (value: string) => void; candidate: { name: string; email: string | null } }) {
  return <select name={name} value={value} onChange={(event) => onChange(event.target.value)} required className="w-full min-w-0 max-w-full rounded-xl border border-ink/10 bg-white px-3 py-2">
...
export function MacIdentityPanel({ candidateId, candidateName, candidateEmail, identity, availableIdentities, isAdmin }: Props) {
  ...
  return <section className="grid gap-3">
    ...
    {isAdmin && identity ? <>
      <Button type="button" variant="secondary" className="w-fit justify-self-start" onClick={() => showControls ? resetControls() : setShowControls(true)}>{showControls ? "Annuler les modifications" : "Gérer l’identité MAC"}</Button>
      {showControls ? <div className="grid min-w-0 gap-5 rounded-2xl border border-ink/10 p-4">
        <div className="grid min-w-0 gap-1"><label htmlFor="mac-identity-search" className="text-sm font-medium">Rechercher une identité</label><input id="mac-identity-search" value={search} onChange={(event) => { setSearch(event.target.value); setLinkIdentityId(""); setMergeIdentityId(""); setLinkConfirmed(false); setMergeConfirmed(false); }} placeholder="Nom, prénom, email, société ou session" className="w-full min-w-0 rounded-xl border border-ink/10 px-3 py-2" /><p className="text-xs text-ink/55">L’identité actuelle de {candidateName || "ce candidat"} est exclue volontairement pour éviter tout auto-rattachement ou toute auto-fusion. Les suggestions visuelles ne réalisent jamais de rapprochement automatique.</p></div>
        ...
        <form action={linkAction} className="grid min-w-0 gap-3">
          ...
          <label className="grid min-w-0 gap-1 text-sm font-medium">Identité canonique<IdentitySelect name="identityId" identities={choices} value={linkIdentityId} onChange={(value) => { setLinkIdentityId(value); setLinkConfirmed(false); }} candidate={candidate} /></label>
          ...
          <Button type="submit" className="w-fit justify-self-start" disabled={linking || !selectedLink || !linkConfirmed}>{linking ? "Rattachement…" : "Confirmer le rattachement"}</Button>{linkState.error ? <p role="alert" className="text-sm text-accent">{linkState.error}</p> : null}{linkState.success ? <p role="status" className="text-sm text-pine">{linkState.success}</p> : null}
        </form>
        <form action={mergeAction} className="grid min-w-0 gap-3 border-t border-ink/10 pt-5">
          ...
          <label className="grid min-w-0 gap-1 text-sm font-medium">Identité canonique<IdentitySelect name="canonicalIdentityId" identities={choices} value={mergeIdentityId} onChange={(value) => { setMergeIdentityId(value); setMergeConfirmed(false); }} candidate={candidate} /></label>
          ...
          <Button type="submit" className="w-fit justify-self-start" disabled={merging || !selectedMerge || !mergeConfirmed}>{merging ? "Regroupement…" : "Confirmer le regroupement"}</Button>{mergeState.error ? <p role="alert" className="text-sm text-accent">{mergeState.error}</p> : null}{mergeState.success ? <p role="status" className="text-sm text-pine">{mergeState.success}</p> : null}
        </form>
      </div> : null}
    </> : null}
  </section>;
