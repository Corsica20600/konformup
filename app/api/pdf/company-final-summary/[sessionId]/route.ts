...
import { CompanyFinalSummaryDocument } from "@/lib/pdf/final-documents";
...
export async function GET(request: Request, context: { params: Promise<{ sessionId: string }> }) {
  ...
  try {
    ...
    const document = createElement(CompanyFinalSummaryDocument as never, {
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        "Content-Disposition": `inline; filename="synthese-finale-${sessionId}.pdf"`
      }
    });
  } catch (error) {
