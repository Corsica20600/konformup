...
import { createQualiopiWatchEntry, QualiopiWatchError, updateQualiopiWatchEntry } from "@/lib/qualiopi-watch";
...
export async function updateQualiopiWatchEntryAction(_: QualiopiWatchActionState, data: FormData): Promise<QualiopiWatchActionState> { try { await updateQualiopiWatchEntry(text(data, "id"), { indicator: text(data, "indicator"), topic: text(data, "topic"), sourceName: text(data, "sourceName"), sourceUrl: text(data, "sourceUrl"), consultedOn: text(data, "consultedOn"), summary: text(data, "summary"), impact: text(data, "impact"), decision: text(data, "decision"), evidenceUrl: text(data, "evidenceUrl"), nextReviewOn: text(data, "nextReviewOn"), status: text(data, "status") }); revalidatePath("/quality-watch"); return { success: "Veille modifiée." }; } catch (error) { return state(error); } }
