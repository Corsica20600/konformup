...
export async function updateQualiopiWatchEntryStatus(id: string, status: string, evidenceUrl: string) { if (!z.string().uuid().safeParse(id).success || !z.enum(statuses).safeParse(status).success || (evidenceUrl && !z.string().url().startsWith("https://").safeParse(evidenceUrl).success)) throw new QualiopiWatchError("Mise à jour invalide."); const value = await context(); const { error } = await value.supabase.from("qualiopi_watch_entries").update({ status: status as QualiopiWatchStatus, evidence_url: evidenceUrl || null }).eq("id", id); if (error) throw new QualiopiWatchError("Impossible de mettre à jour la preuve."); }
