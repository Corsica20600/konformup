...
export function buildQuoteEmailBody(quote: QuoteEditData, signatureLines: string[]) {
  const agreementNote =
...
export function buildQuoteEmailHtml(quote: QuoteEditData) { const title = escapeHtml(getTrainingDocumentTitle(quote.training_type, quote.title)); return `<p>Bonjour,</p><p>Veuillez trouver ci-joint notre devis <strong>${escapeHtml(quote.quote_number)}</strong> relatif a votre demande de formation.</p><p>Ce devis concerne : ${title}.</p><p>Nous restons a votre disposition pour toute question ou pour convenir des prochaines etapes.</p>`; }
...
export async function sendQuoteEmail(quote: QuoteEditData) {
  ...
  await sendBrevoTransactionalEmail({
    ...
    textContent: body,
    htmlContent: buildQuoteEmailHtml(quote),
    attachment: [
        {
